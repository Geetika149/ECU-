...
tempRange = [600, 1000];         % Temperature range (600 to 1000 °C)
pressureRange = [200, 400];      % Pressure range (200 to 400 kPa)
rpmRange = [20000, 100000];      % RPM range (20000 to 100000)

% Simulate random sensor data
time = 0:0.1:100;  % Simulate for 100 seconds at 0.1s intervals
temp = tempRange(1) + (tempRange(2) - tempRange(1)) * rand(size(time));  % Random temperature data
pressure = pressureRange(1) + (pressureRange(2) - pressureRange(1)) * rand(size(time));  % Random pressure data
rpm = rpmRange(1) + (rpmRange(2) - rpmRange(1)) * rand(size(time));  % Random RPM data
...
subplot(5,1,5);
scatter(time, engineConditions, 'filled'); title('Engine Conditions'); ylabel('Condition');
yticks(1:5);
...
function [engineCondition, fuelRateAdjustment] = analyzeEngineCondition(temp, pressure, rpm)
    ...
    switch engineCondition
        case 'Overheating'
            fuelRateAdjustment = -0.1;  % Decrease fuel rate by 10%
        case 'Low Pressure'
            fuelRateAdjustment = -0.1;  % Decrease fuel rate by 10%
        case 'High Pressure'
